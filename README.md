...
完整触发模型与默认配置见 [DESIGN.md §3](DESIGN.md)。
...
**更新版本后按键失灵？** 多半是 TCC 权限失效。正式分发包用固定 `MiRemote Dev` 证书 +
固定 bundle id 签名，正常重编译不会掉权限；只有证书/身份变化才需要在「系统设置 → 隐私与
安全性 → 输入监控 / 辅助功能」里删掉旧条目重新授权。用临时（ad-hoc）签名则每次重编译都会掉，
所以 `package.sh` 在缺证书时会硬失败而非回退。
...
4. 遥控器连不上：长按 **主页+返回 3 秒**至指示灯闪烁进入配对，在系统蓝牙里点连接；
