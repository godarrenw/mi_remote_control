...
- 实机操作收口修正：“所有 App”窗口选择通过 NSWorkspace 协作式激活交接；聚焦输入改为可见候选评分，编辑区优先于侧栏搜索。
- Home 单按立即进入调度中心（无双击等待）；随后左右键切换桌面，Home 或菜单再按发 Esc 退出，20s 无操作自动退出按键接管态。v7 会自动迁移旧配置。
- 应用形态改为 Dock + 菜单栏双入口：运行时 Dock 图标常驻，关闭设置窗口不退出服务，再点 Dock 图标会重开设置窗口。
- 系统功能菜单删除了与实体音量键重复且不对称的“音量＋”，保留遥控器没有独立键的“静音”；常规功能区重排为 3×3。
- 支持 App 的默认映射不再覆盖 Home/菜单基础语义：任何 App 中默认都是 Home=调度中心/切桌面、菜单=窗口选择/系统功能；聚焦编辑区、地址栏、字幕等 App 专属动作搬到 TV 控制模式。v6 迁移只清理内置支持 App，用户自建 Profile 不改写。
- 菜单栏图标无单独系统授权；若“显示菜单栏图标”开启，StatusItemController 在设置 autosaveName 后会强制恢复 isVisible，不再被历史 Cmd+拖除的隐藏记录永久卡住。
- 窗口选择器的菜单键改为三段流程：第一下优先打开全局所有 App/全部桌面，第二下切到当前 App，第三下关闭。全局数据源从 `optionOnScreenOnly` 改为 `optionAll + excludeDesktopElements`，并过滤 helper/菜单栏小窗口。
